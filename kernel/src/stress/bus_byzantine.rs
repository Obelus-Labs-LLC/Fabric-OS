...
fn test1_basic_send_receive() -> StressResult {
    cleanup();
...
fn test2_capability_validation() -> StressResult {
    cleanup();
...
fn test3_sequence_enforcement() -> StressResult {
    cleanup();
    let cap_id = setup_pair(1, 2);
    let mut correct = 0u32;
    let total = 4u32;
...
fn test4_hmac_integrity() -> StressResult {
    cleanup();
    let cap_id = setup_pair(1, 2);
...
fn test5_bus_flood_backpressure() -> StressResult {
    cleanup();
...
fn test6_monitor_tap() -> StressResult {
    cleanup();
...
fn test7_audit_hash_chain() -> StressResult {
    cleanup();
    let cap_id = setup_pair(1, 2);
    const BATCH: u32 = 30;
...
fn test8_byzantine_message_reject() -> StressResult {
    cleanup();
    let mut correct = 0u32;
    let total = 6u32;
