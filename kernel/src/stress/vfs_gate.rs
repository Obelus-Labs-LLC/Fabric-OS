//! STRESS Phase 8 Gate — VFS + Filesystem verification tests.
//!
...
fn test_inode_table_crud() -> StressResult {
...
fn test_mount_resolve() -> StressResult {
...
fn test_tmpfs_read_write() -> StressResult {
    use crate::vfs::inode::InodeType;
...
fn test_tmpfs_directory_ops() -> StressResult {
    use crate::vfs::inode::InodeType;
...
fn test_devfs_null() -> StressResult {
...
fn test_devfs_zero() -> StressResult {
    let mut devfs = crate::vfs::DEVFS.lock();
    if !devfs.is_initialized() {
        return StressResult {
...
fn test_devfs_random() -> StressResult {
    let mut devfs = crate::vfs::DEVFS.lock();
    if !devfs.is_initialized() {
        return StressResult {
...
fn test_vfs_open_read_close() -> StressResult {
    use crate::vfs::ops;
    ...
    // Create a test file in tmpfs via the VFS
    {
        ...
        // Write test data
        let test_data = b"STRESS test data 8";
    ...
    if &buf[..read] != b"STRESS test data 8" {
        return StressResult {
            test_name: "Syscall Open/Read/Close",
            passed: false, score: 0, weight: 15,
...
fn test_stdio_pre_open() -> StressResult {
    use crate::vfs::open_file::OpenFlags;
...
fn test_cpio_parse_load() -> StressResult {
    use crate::vfs::cpio;
