...
fn test1_mass_creation() -> StressResult {
...
fn test2_validation_throughput() -> StressResult {
    capability::STORE.lock().clear();
    let mut errors = 0u32;
...
fn test3_hmac_tamper_detection() -> StressResult {
    capability::STORE.lock().clear();
    let mut errors = 0u32;
...
fn test4_delegation_chain_depth() -> StressResult {
    capability::STORE.lock().clear();
...
fn test5_permission_escalation() -> StressResult {
    capability::STORE.lock().clear();
...
fn test6_budget_enforcement() -> StressResult {
    capability::STORE.lock().clear();
    let mut correct = 0u32;
    let total = 3u32;
...
fn test7_nonce_replay() -> StressResult {
    capability::STORE.lock().clear();
    let mut correct = 0u32;
    let total = 4u32;
...
fn test8_expiration() -> StressResult {
    capability::STORE.lock().clear();
    let mut correct = 0u32;
    let total = 2u32;
...
fn test9_revocation_storm() -> StressResult {
    capability::STORE.lock().clear();
    let mut errors = 0u32;
