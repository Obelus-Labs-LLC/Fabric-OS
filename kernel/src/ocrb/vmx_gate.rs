//! OCRB Phase 17 Gate — VMX Foundation Tests
//!
...
fn test_cpuid_vendor() -> OcrbResult {
...
fn test_cpuid_vmx_coherent() -> OcrbResult {
    let features = cpuid::probe_features();
...
fn test_ept_lifecycle() -> OcrbResult {
...
fn test_ept_isolation() -> OcrbResult {
...
fn test_vmcs_fields() -> OcrbResult {
...
fn test_emulator_hlt() -> OcrbResult {
...
fn test_emulator_cpuid_filter() -> OcrbResult {
    let mut vt = VmTable::new();
...
fn test_emulator_io_port() -> OcrbResult {
    let mut vt = VmTable::new();
    let vm_id = match vt.create(1) {
        Ok(id) => id,
        Err(_) => return OcrbResult {
...
fn test_vm_lifecycle() -> OcrbResult {
    let mut vt = VmTable::new();
...
fn test_vm_table_limits() -> OcrbResult {
    let mut vt = VmTable::new();
